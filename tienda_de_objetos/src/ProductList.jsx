...
export default function ProductList({ onProductDetail, onRegisterProduct }) {
  ...
  return (
    <div style={{ maxWidth: 700, margin: "2rem auto", padding: "2rem", background: "#fff", borderRadius: 8, boxShadow: "0 2px 8px #0002" }}>
      ...
      />
      <button style={{ marginBottom: "1rem" }} onClick={onRegisterProduct}>Registrar nuevo producto</button>
        ...
        <tbody>
          {paginated.map(p => (
            <tr key={p.id} style={{ background: p.activo ? "#fff" : "#fee" }}>
              ...
              <td>
                <button onClick={() => onProductDetail && onProductDetail(p)} style={{ marginRight: "0.5rem" }}>Ver detalle</button>
                {p.activo ? (
